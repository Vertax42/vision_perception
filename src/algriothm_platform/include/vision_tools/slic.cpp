...
{
    double dc = sqrt(pow(centers[ci][0] - colour.val[0], 2) + pow(centers[ci][1] - colour.val[1], 2)
            ...
            for(int k = centers[j][3] - step; k < centers[j][3] + step; k++)
            {
                for(int l = centers[j][4] - step; l < centers[j][4] + step; l++)
                {
                    ...
                    if(k >= 0 && k < image.cols && l >= 0 && l < image.rows)
                    {
                        ...
                        Vec3b color = image.at<Vec3b>(l, k);
                        cv::Scalar colour;
                        colour.val[0] = color[0];
                        colour.val[1] = color[1];
                        colour.val[2] = color[2];
...
void Slic::display_contours(cv::Mat &image, cv::Scalar colour)
{
    ...
    for(int i = 0; i < (int)contours.size(); i++)
    {
        ...
        Vec3b color;
        color[0] = colour.val[0];
        color[1] = colour.val[1];
        color[2] = colour.val[2];
...
{
    vector<Vec3b> colours(centers.size(), Vec3b(0, 0, 0));
    ...
    for(int i = 0; i < image.cols; i++)
    {
        for(int j = 0; j < image.rows; j++)
        {
            Vec3b ncolour = colours[clusters[i][j]];
